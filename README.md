...
If this project is on GitHub, use the repository **Code** button to copy its clone URL, then clone it and enter the project folder. If you downloaded the release ZIP, extract it and open the extracted `QuickTalk-v1.0-Release` folder.

### 2. Create MongoDB Atlas access

Create a MongoDB database user and allow your development IP in Atlas. QuickTalk uses the database name:

```text
quicktalk
```

Do not put the real MongoDB password in `application.properties`, `.env.example`, screenshots, commits, or issue reports.
...
```powershell
$env:MONGODB_URI='mongodb+srv://USERNAME:PASSWORD@YOUR_CLUSTER_HOST/?retryWrites=true&w=majority&appName=QuickTalk'
```

Windows Command Prompt:

```bat
set MONGODB_URI=mongodb+srv://USERNAME:PASSWORD@YOUR_CLUSTER_HOST/?retryWrites=true^&w=majority^&appName=QuickTalk
```

In IntelliJ IDEA, you can instead add `MONGODB_URI` under **Run > Edit Configurations > Environment variables**.
...
The v1.0 release test set covers the Spring application context, input validation, rate limiting, message ownership, reaction toggling, and deletion behavior. The context test disables MongoDB index creation and uses a local test URI, so it does not require your real Atlas password.
